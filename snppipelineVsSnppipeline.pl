...
sub main{
  my $settings={};
  GetOptions($settings,qw(help snppipeline=s ref|reference=s));
  ...
  my $refSnps=snppipelineSnps($$settings{ref},$settings);
  
  # compare
  my($TP,$TN,$FP,$FN)=compareSnps($snppipelineSnps,$refSnps,$settings);

  # Report
  print join("\t",qw(S-P TTR TP TN FP FN))."\n";
  print join("\t",$$settings{snppipeline},$$settings{ref},$TP, $TN, $FP, $FN)."\n";
}
...
sub usage{
  ...
  Usage: $0 --snpPipeline projdirectory --ref referenceSnpPipeline
  "
}
