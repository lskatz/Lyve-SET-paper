...
qsub -q all.q -N RealPhy -o $TMP/log -j y -pe smp 12,16 -V -S /bin/bash -cwd -t 1-$(cat $CTRL_FILE | wc -l) \
...
" > $scratch_out/out/config.txt
  ...
  #REALPHY_v112 $scratch_out/samples $scratch_out/out -readLength 250 -ref reference -polyThreshold 0.75 -perBaseCov 4
