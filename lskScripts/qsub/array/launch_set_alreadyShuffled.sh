...
export PATH=$HOME/bin/cg_pipeline/scripts:$PATH
...
mkdir -p $TMP/log
qsub -q all.q -N LyveSetShuffled -o $TMP/log -j y -pe smp 12,16 -V -cwd -t 1-$(cat $CTRL_FILE | wc -l) \
  -v "CTRL_FILE=$CTRL_FILE" -v "REF=$REF" <<- "END_OF_SCRIPT"
  #!/bin/bash

  base_dir=$(sed -n ${SGE_TASK_ID}p $CTRL_FILE)
  echo "$(hostname): Working on $base_dir with reference file $REF"
  ...
  ln -sv $(realpath $base_dir/reads)/*.f*q.gz $scratch_out/reads/
  if [ $? -gt 0 ]; then exit 1; fi;
  #launch_set.pl --noqsub --numcpus $NSLOTS --read_cleaner CGP --mask-phages --mask-cliffs $scratch_out
  launch_set.pl --noqsub --numcpus $NSLOTS --read_cleaner CGP --presets listeria_monocytogenes $scratch_out
  if [ $? -gt 0 ]; then exit 1; fi;
