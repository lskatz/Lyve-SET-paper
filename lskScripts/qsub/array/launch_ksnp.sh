...
mkdir -p $TMP/log
qsub -q all.q -N KSNP3 -o $TMP/log -j y -pe smp 12,16 -V -S /bin/bash -cwd -t 1-$(cat $CTRL_FILE | wc -l) \
  ...
  echo $READS | xargs -P $NSLOTS -n 1 sh -c '
    ...
    mv $sampleDir/$sample.fasta $scratch_out/samples/$sample.fasta
    rm -rvf $sampleDir
  ';
  ...
  pushd $scratch_out
    ...
    sed -i.bak '/reference/d' in.txt

    kSNP3 -k $KMERLENGTH -annotate reference_in.txt -all_annotations -in in.txt -core -ML -min_frac 0.75 -CPU $NSLOTS -NJ -vcf -outdir out
